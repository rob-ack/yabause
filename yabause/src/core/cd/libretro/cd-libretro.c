...
static int LoadBinCueInZip(const char *filename, RFILE *fp)
{
  ...
  if (cue == NULL) return -1;
   ...
   for (;;)
   {
       const char * const data = (const char* const)&data[index];
      // Retrieve a line in cue
      if (sscanf(data, "%s%n", temp_buffer, &pos) == EOF)
         break;
      index+= pos;
...
static int GetIntCCD(ccd_struct const *ccd, char const *section, char const *name)
{
	int i;
...
static int ISOCDInit(const char * iso) {
   ...
   {
      YabSetError(YAB_ERR_FILENOTFOUND, (char const *)iso);
      return -1;
   }
...
static int LoadCHD(const char *chd_filename, RFILE *iso_file)
{
  ...
  while ( chd_get_metadata(pChdInfo->chd, 0, num_tracks, buf, meta_outlen, &resultlen, &resulttag, &resultflags) == CHDERR_NONE )  {
    ...
    case CDROM_TRACK_METADATA_TAG:
      sscanf((char const*)buf, CDROM_TRACK_METADATA_FORMAT, &trak_number, track_type, track_subtype, &frame);
      pregap = 0;
      postgap = 0;
      sprintf(pg_type, "NONE");
      break;
    case CDROM_TRACK_METADATA2_TAG:
      sscanf((char const*)buf, CDROM_TRACK_METADATA2_FORMAT, &trak_number, track_type, track_subtype, &frame, &pregap, pg_type, pg_sub_type, &postgap);
      break;
    default:
      return -1;
    }
