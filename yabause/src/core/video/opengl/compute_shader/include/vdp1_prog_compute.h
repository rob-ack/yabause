...
"  vec2 A = V0;\n"
"  vec2 B = vec2(V1.x, V0.y);\n"
"  if (A.x <= B.x) B.x += upscale.x;\n"
"  else A.x += upscale.x;\n"
"  for (uint i=0; i<step; i++) {\n"
"    vec3 d = antiAliasedPoint(P+vec2(0.5, 0.0), A, B);\n"
"    if ((distance(d.xy, P+vec2(0.5, 0.0)) <= length(upscale)/2.0) && ((d.z>=0.0) && (d.z<=1.0))) {\n"
"      float ux= d.z;\n" //Pb avec Dz il est trop haut... il faut qu'il renvoie entre [0.0, 1.0[
...
" uint y = uint(ceil(uv.y*(pixcmd.h-1)));\n"
// " posf = ceil(posf);\n"
" if ((pixcmd.flip & 0x2u) == 0x2u) y = (pixcmd.h-1) - uint(floor(uv.y*(pixcmd.h-1)));\n"

" uint x = uint(uv.x*(pixcmd.w-1));\n"
" if ((pixcmd.flip & 0x1u) == 0x1u) x = (pixcmd.w-1) - x;\n"
