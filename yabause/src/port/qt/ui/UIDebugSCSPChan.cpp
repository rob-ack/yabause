...
UIDebugSCSPChan::UIDebugSCSPChan(QWidget * p) : QDialog(p)
{
    ...
    connect(timer, &QTimer::timeout, this, &UIDebugSCSPChan::update_window);
...
void UIDebugSCSPChan::paintEvent(QPaintEvent * event)
{
    ...
    for (int i = 0; i < 32; i++)
    {
        int constexpr spacer = 4;
        double constexpr max_height = 64;
        int constexpr channel_width = 16;
        int constexpr start_x = 64;

        int env = 0, state = 0;
        scsp_debug_get_envelope(i, &env, &state);

        double const env_ratio = (1023.0 - env) / 1023.0;
        QRect rect = QRect(start_x + space + (i * channel_width), 8, channel_width, (int)(max_height * env_ratio));
