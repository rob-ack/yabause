...
static u16 FASTCALL FetchBios(u32 addr)
{
...
static u16 FASTCALL FetchLWram(u32 addr)
{
	return SH2MappedMemoryReadWord(NULL, addr);
}
...
static u16 FASTCALL FetchHWram(u32 addr)
{
	return SH2MappedMemoryReadWord(NULL, addr);
}
...
static u16 FASTCALL FetchCs0(u32 addr)
{
    return SH2MappedMemoryReadWord(NULL, addr);
}
      ...
      if (((i>>8) == 0x0) || ((i>>8) == 0x2)) {
        switch (i&0xFF)
        {
          ...
          case 0x002: // Low Work Ram
              krfetchlist[i] = FetchLWram;// SH2MappedMemoryReadWord;
            cacheId[i] = 1;
            break;
          case 0x020: // CS0
              krfetchlist[i] = FetchCs0; //SH2MappedMemoryReadWord;
            cacheId[i] = 2;
            break;
          case 0x05c: // Fighting Viper
              krfetchlist[i] = FetchVram;//SH2MappedMemoryReadWord;
            cacheId[i] = 3;
            break;
          ...
          case 0x06E:
          case 0x06F:
              krfetchlist[i] = FetchHWram;// SH2MappedMemoryReadWord;
            cacheId[i] = 4;
            break;
          default:
            krfetchlist[i] = FetchInvalid;
...
void SH2KronosInterpreterTestExec(SH2_struct *context, u32 cycles)
{
  u32 target_cycle = context->cycles + cycles;
  if (context == MSH2)
...
void SH2KronosInterpreterAddCycles(SH2_struct *context, u32 value)
{
