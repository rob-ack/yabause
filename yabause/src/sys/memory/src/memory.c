...
{
  if ((context != NULL) && (mem == *MemoryBuffer[(addr >> 16) & 0xFFF])){
    context->cycles +=10; //Done
  }
...
{
  if ((context != NULL) && (memory == *MemoryBuffer[(addr >> 16) & 0xFFF])){
    context->cycles += 10; //Done
  }
