...
extern "C" Vdp2 * fixVdp2Regs = NULL;
//#define PERFRAME_LOG
#ifdef PERFRAME_LOG
int fount = 0;
FILE *ppfp = NULL;
...
extern "C" int VIDOGLInit(void);
extern "C" void VIDOGLDeInit(void);
extern "C" void VIDOGLResize(int, int, unsigned int, unsigned int, int, int);
extern "C" int VIDOGLIsFullscreen(void);
extern "C" int VIDOGLVdp1Reset(void);
extern "C" void VIDOGLVdp1DrawStart(void);
extern "C" void VIDOGLVdp1DrawEnd(void);
...
extern "C" int VIDOGLVdp2Reset(void);
extern "C" void VIDOGLVdp2DrawStart(void);
extern "C" void VIDOGLVdp2DrawEnd(void);
extern "C" void VIDOGLVdp2DrawScreens(void);
extern "C" void VIDOGLVdp2SetResolution(u16 TVMD);
extern "C" void YglGetGlSize(int *width, int *height);
extern "C" void VIDOGLGetNativeResolution(int *width, int *height, int*interlace);
extern "C" void VIDOGLVdp1ReadFrameBuffer(u32 type, u32 addr, void * out);
extern "C" void VIDOGLVdp1WriteFrameBuffer(u32 type, u32 addr, u32 val);
extern "C" void VIDOGLSetSettingValueMode(int type, int value);
extern "C" void VIDOGLSync();
extern "C" void VIDOGLGetNativeResolution(int *width, int *height, int*interlace);
extern "C" void VIDOGLVdp2DispOff(void);
extern "C" void VIDOGLOnUpdateColorRamWord(u32 addr);
extern "C" void VIDOGLVulkanGetScreenshot(void ** outbuf, int * width, int * height) { return;  }

static int vdp1_interlace = 0;

extern "C" float vdp1wratio = 1;
extern "C" float vdp1hratio = 1;

extern "C" int GlWidth = 320;
extern "C" int GlHeight = 224;

extern "C" int vdp1cor = 0;
extern "C" int vdp1cog = 0;
extern "C" int vdp1cob = 0;
...
extern "C" vdp2rotationparameter_struct  paraA = { 0 };
extern "C" vdp2rotationparameter_struct  paraB = { 0 };
          ...
          *texture->textdata++ = VDP1COLOR(0, 1, priority, 1, 0);
          ...
          if (dot & 0x8000 && (fixVdp2Regs->SPCTL & 0x20) ) {
             *texture->textdata++ = VDP1COLOR(0, colorcl, priority, 0, VDP1COLOR16TO24(dot));
              ...
              u16 arg2 = dot;
            Vdp1MaskSpritePixel(fixVdp2Regs->SPCTL & 0xF, &arg2, &colorcl);
...
extern "C" void VIDOGLResize(int originx, int originy, unsigned int w, unsigned int h, int on, int aspect_rate_mode)
{
  ...
  _Ygl->aspect_rate_mode = (ASPECT_RATE_MODE)aspect_rate_mode;
  YglGLInit(2048, 1024);
...
extern "C" void VIDOGLVdp1NormalSpriteDraw(vdp1cmd_struct * const _cmd, u8 * ram, Vdp1 * regs, u8* back_framebuffer)
{
  vdp1cmd_struct & cmd = *_cmd;
  YglSprite sprite;
  YglTexture texture;
  YglCache cash;
  u64 tmp;
...
//  if ((CMDYA & 0x1000)) CMDYA |= 0xE000; else CMDYA &= ~(0xE000);
  ...
  sprite.vertices[0] = (int)((float)cmd.CMDXA * vdp1wratio);
  sprite.vertices[1] = (int)((float)cmd.CMDYA * vdp1hratio);
  sprite.vertices[2] = (int)((float)(cmd.CMDXA + sprite.w) * vdp1wratio);
  sprite.vertices[3] = (int)((float)cmd.CMDYA * vdp1hratio);
  sprite.vertices[4] = (int)((float)(cmd.CMDXA + sprite.w) * vdp1wratio);
  sprite.vertices[5] = (int)((float)(cmd.CMDYA + sprite.h) * vdp1hratio);
  sprite.vertices[6] = (int)((float)cmd.CMDXA * vdp1wratio);
  sprite.vertices[7] = (int)((float)(cmd.CMDYA + sprite.h) * vdp1hratio);
  ...
  if ((cmd.CMDPMOD & 4) == 0) col = nullptr; // No Gouraud shading, use same color for all 4 vertices
...
extern "C" void VIDOGLVdp1ScaledSpriteDraw(vdp1cmd_struct* const _cmd, u8 * ram, Vdp1 * regs, u8* back_framebuffer)
{
  vdp1cmd_struct & cmd = *_cmd;
  YglSprite sprite;
  YglTexture texture;
...
//  sprite.vertices[7] = (int)((float)(y + rh) * vdp1hratio);
  ...
  sprite.vertices[0] = cmd.CMDXA;
  sprite.vertices[1] = cmd.CMDYA;
  sprite.vertices[2] = cmd.CMDXB;
  sprite.vertices[3] = cmd.CMDYB;
  sprite.vertices[4] = cmd.CMDXC;
  sprite.vertices[5] = cmd.CMDYC;
  sprite.vertices[6] = cmd.CMDXD;
  sprite.vertices[7] = cmd.CMDYD;
...
extern "C" void VIDOGLVdp1DistortedSpriteDraw(vdp1cmd_struct * const _cmd, u8 * ram, Vdp1 * regs, u8* back_framebuffer)
{
  vdp1cmd_struct & cmd = *_cmd;
  YglSprite sprite;
  YglTexture texture;
...
//  }
  ...
  for (int i = 0; i < 4; i++) {
      sprite.vertices[2 * i] *= vdp1wratio;
      sprite.vertices[2 * i + 1] *= vdp1hratio;
  }
...
extern "C" void VIDOGLVdp1PolygonDraw(vdp1cmd_struct* const _cmd, u8 * ram, Vdp1 * regs, u8* back_framebuffer)
{
  u16 color;
  YglSprite sprite;
  YglTexture texture;
...
extern "C" void VIDOGLVdp1PolylineDraw(vdp1cmd_struct* const _cmd, u8 * ram, Vdp1 * regs, u8* back_framebuffer)
{
  s16 v[8];
  float line_poygon[8];
  u16 color;
...
extern "C" void VIDOGLVdp1LineDraw(vdp1cmd_struct* const _cmd, u8 * ram, Vdp1 * regs, u8* back_framebuffer)
{
  s16 v[4];
  u16 color;
  u16 CMDPMOD;
...
extern "C" void VIDOGLVdp1UserClipping(vdp1cmd_struct* cmd, u8 * ram, Vdp1 * regs)
{
...
extern "C" void VIDOGLVdp1SystemClipping(vdp1cmd_struct* cmd, u8 * ram, Vdp1 * regs)
{
...
extern "C" void VIDOGLVdp1LocalCoordinate(vdp1cmd_struct* cmd, u8 * ram, Vdp1 * regs)
{
...
extern "C" void VIDOGLVdp2DispOff()
{
}
...
extern "C" void VIDOGLSetFilterMode(int type) {
  _Ygl->aamode = (AAMODE)type;
  return;
}
...
extern "C" void VIDOGLSetSettingValueMode(int type, int value) {
  ...
  case VDP_SETTING_RBG_RESOLUTION_MODE:
    _Ygl->rbg_resolution_mode = (RBG_RESOLUTION_MODE)value;
    break;
